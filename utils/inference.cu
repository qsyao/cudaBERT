...
{
>>>>>>> 5c82c55b75c439d7624979abda314f36b2398cb8:cuda/utils/inference.cu
...
Retval BERT_Inference (global_manager * handle,
                    ...
                    int* attention_mask) {
=======
extern "C" Retval BERT_Inference(global_manager *handle,
                                 int *words,
                                 int *token_types,
                                 int batchsize,
                                 int seq_length,
...
>>>>>>> 5c82c55b75c439d7624979abda314f36b2398cb8:cuda/utils/inference.cu
    ...
    debug_tensor_gpu<float>(std::string("pooler_output"), ret.pooled_output, 10, handle->hidden_size);
                      ...
                      size_t num_classes)
{
    ...
    size_t hidden_size = handle->hidden_size;
    T *linear_output;
    linear_output = handle->global_malloc_manage_float.get_new_head_point(batchsize * num_classes);

    std::vector<std::string> keys = {"classifier_weight"};
    T *weights = look_up_tts(handle->tts, keys)->gpu_mem;
